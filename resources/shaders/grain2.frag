...
void main() {
    ...
    //gl_FragColor =  vec4(grain(u_texture, st, v_resolution, 20.5*cos(.4*u_time)), 1.);
    //gl_FragColor = vec4(grain(u_texture, st, v_resolution, 20.5*cos(.4*u_time)), 1.);
    ...
    gl_FragColor = v_color * texture2D(u_texture, v_texCoords) - st.g * 0.3 + st.r * 0.3; // smaller stuff
    // gl_FragColor = v_color * texture2D(u_texture, v_texCoords) - st.g * 0.1 + st.r * 0.1; // whole screen
    // gl_FragColor = (v_color + vec4(-st.x, -st.y, 0.0, 0.0) * 0.5) * texture2D(u_texture, v_texCoords);
}
